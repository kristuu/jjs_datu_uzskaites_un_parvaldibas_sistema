<template>
  <div>
    <AdminTable
      v-show="can('manage cities')"
      :database-table="'cities'"
      :instance-id-column="'id'"
      :model-name="'City'"
      :page-name="$t(`pageHeadings.cities.manage cities`)"
      :short-desc="$t(`pageHeadings.cities.total`, { total: totalInstances })"
    >
      <DataTable
        ref="dt"
      ...
      >
        <template #header>
          ...
          >
            <Button
              ...
              raised
              rounded
              @click="router.push({ name: 'CreateCity' })"
            >
            </Button>
        ...
        <Column :header="$t('table.cities.name')" field="name" sortable>
          <template #body="{ data }">
            {{ data.name }}
          </template>
          <template #filter="{ filterModel }">
        ...
        <Column
          :header="$t('table.cities.country.name')"
          field="country.name"
        ...
        <Column :exportable="false">
          <template #body="{ data }">
            <Button
              ...
              outlined
              rounded
              @click="fetchDataStore.deleteInstance(`cities`, data.id)"
            />
          </template>
        </Column>
      </DataTable>
        ...
        <div class="flex flex-column h-full container">
          ...
          <div class="mt-auto">
            ...
            <div class="m-3 flex justify-content-between gap-3 text-primary">
              <router-link
                v-if="instance.id"
                :to="{ name: `EditCity`, params: { id: instance.id } }"
                @click="visible = false"
              >
              ...
              <span
                ...
                @click="
                  () => {
                    visible = false;
                    fetchDataStore.deleteInstance(`cities`, instance.id);
                  }
                "
...
const onRowSelect = async (event) => {
  await fetchDataStore.fetchInstance("cities", event.data.id);
  visible.value = true;
};
...
onBeforeMount(async () => {
  await fetchDataStore.fetchDatabaseData("cities");
});
</script>
