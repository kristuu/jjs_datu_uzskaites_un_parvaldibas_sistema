<template>
  <div>
    <AdminTable
      ...
      :short-desc="
        $t(`pageHeadings.countries.total`, { total: totalInstances })
      "
    >
      <DataTable
      ...
      >
        ...
        <Column :header="$t('table.countries.name')" field="name" sortable>
          <template #body="{ data }">
            {{ data.name }}
          </template>
          <template #filter="{ filterModel }">
        ...
        <Column :exportable="false">
          <template #body="{ data }">
            ...
            <Button
              ...
              outlined
              rounded
              @click="fetchDataStore.deleteInstance(`countries`, data.id)"
            />
          </template>
        </Column>
      </DataTable>
        ...
        <div class="flex flex-column h-full container">
          ...
          <div class="mt-auto">
            ...
            <div class="m-3 flex justify-content-between text-primary">
              <router-link
                v-if="instance.id"
                :to="{ name: `EditCountry`, params: { id: instance.id } }"
                @click="visible = false"
              >
              ...
              <span
                ...
                @click="
                  () => {
                    fetchDataStore.deleteInstance(`countries`, instance.id);
                    visible = false;
                  }
                "
