<template>
  <div>
    <AdminTable
      v-show="can('manage event_categories')"
      :database-table="'event_categories'"
      :instance-id-column="'id'"
      :model-name="'EventCategory'"
      :page-name="$t(`pageHeadings.event_categories.manage event_categories`)"
      :short-desc="
        $t(`pageHeadings.event_categories.total`, { total: totalInstances })
      "
    >
      <DataTable
      ...
      >
        <template #header>
          ...
          >
            <Button
              ...
              raised
              rounded
              @click="router.push({ name: 'CreateEventCategory' })"
            >
            </Button>
        ...
        <Column
          :header="$t('table.event_categories.name')"
          field="name"
          sortable
        >
          <template #body="{ data }">
        ...
        <Column :exportable="false">
          <template #body="{ data }">
            <Button
              ...
              rounded
              @click="
                fetchDataStore.deleteInstance(`event_categories`, data.id)
              "
            />
          </template>
        </Column>
        ...
        <div class="flex flex-column h-full container">
          ...
          >
            ...
            <span class="font-semibold text-2xl text-primary">{{
              $t(`details_sidebar.event_category`)
            }}</span>
            <Button
              class="h-2rem w-2rem"
              icon="pi pi-times"
          ...
          <div class="overflow-y-auto w-100">
            <div class="row gap-3 container-fluid mx-auto">
              ...
              <div class="d-flex flex-column col-12 sm:col-6 lg:col-3">
                <label>{{ $t(`table.event_categories.name`) }}</label>
          ...
          <div class="mt-auto">
            ...
            <div class="m-3 flex justify-content-between gap-3 text-primary">
              <router-link
                v-if="instance.id"
                :to="{ name: `EditEventCategory`, params: { id: instance.id } }"
                @click="visible = false"
              >
...
const onRowSelect = async (event) => {
  await fetchDataStore.fetchInstance("event_categories", event.data.id);
  visible.value = true;
};

onBeforeMount(async () => {
  await fetchDataStore.fetchDatabaseData("event_categories");
});
</script>
