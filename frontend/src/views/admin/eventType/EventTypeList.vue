<template>
  <div>
    <AdminTable
      v-show="can('manage event_types')"
      :database-table="'event_types'"
      :instance-id-column="'id'"
      :model-name="'EventType'"
      :page-name="$t(`pageHeadings.event_types.manage event_types`)"
      :short-desc="
        $t(`pageHeadings.event_types.total`, { total: totalInstances })
      "
    >
      <DataTable
      ...
      >
        ...
        <Column :header="$t('table.event_types.name')" field="name" sortable>
          <template #body="{ data }">
            {{ data.name }}
          </template>
          <template #filter="{ filterModel }">
        ...
        <Column :exportable="false">
          <template #body="{ data }">
            <Button
              ...
              outlined
              rounded
              @click="fetchDataStore.deleteInstance(`event_types`, data.id)"
            />
          </template>
        </Column>
      </DataTable>
        ...
        <div class="flex flex-column h-full container">
          ...
          <div class="overflow-y-auto w-100">
            <div class="row gap-3 container-fluid mx-auto">
              ...
              <div class="d-flex flex-column col-12 sm:col-6 lg:col-3">
                <label>{{ $t(`table.event_types.name`) }}</label>
          ...
          <div class="mt-auto">
            ...
            <div class="m-3 flex justify-content-between gap-3 text-primary">
              <router-link
                v-if="instance.id"
                :to="{ name: `EditEventType`, params: { id: instance.id } }"
                @click="visible = false"
              >
              ...
              <span
                ...
                @click="
                  () => {
                    fetchDataStore.deleteInstance(`event_types`, instance.id);
                    visible = false;
                  }
                "
...
const onRowSelect = async (event) => {
  await fetchDataStore.fetchInstance("event_types", event.data.id);
  visible.value = true;
};

onBeforeMount(async () => {
  await fetchDataStore.fetchDatabaseData("event_types");
});
</script>
