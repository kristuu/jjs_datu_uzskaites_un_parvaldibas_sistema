...
const resolveBreadcrumb = (name) => {
  routeTranslations.value =
    locale.locale.value === "lv" ? routeTranslationsLV : routeTranslationsEN;
      ...
      if (breadcrumb) {
        breadcrumb = breadcrumb.map((breadcrumb, i, arr) => {
          ...
          return {
            ...
            isActive:
              i === arr.length - 1 &&
              matchedRoute === matchedRoutes[matchedRoutes.length - 1],
          };
        });
